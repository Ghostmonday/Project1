...
// MARK: - App Configuration & Logging
struct AppLogger {
...
// CLEANED: Logic from Models/DomainModels.swift now in separate Models.swift file.
// Models are imported from NeurospLIT/Models/Domain/Models.swift

// Legacy aliases for compatibility
typealias OffTheTop = OffTheTopRule

struct SplitResult { var splits: [Participant]; var warnings: [String] }

// MARK: - Core Errors
// Errors are imported from NeurospLIT/Models/Supporting/Errors.swift

// MARK: - Core Engine
// Engine logic is in NeurospLIT/Engine/Engine.swift

// Wrapper for SplitResult compatibility
func computeSplitsWrapper(template: TipTemplate, pool: Double) -> SplitResult {
    do {
        let (splits, warnings) = try computeSplits(template: template, pool: pool)
...
// Implementation is in NeurospLIT/Engine/Engine.swift (public functions)
// The legacy in-app engine implementation has been removed

// MARK: - Minimal Helpers Reintroduced
// Legacy disabled code removed
#if false
...
func computeSplitsCompat(template: TipTemplate, pool: Double) -> (splits: [Participant], warnings: [String]) {
    let r = computeSplitsWrapper(template: template, pool: pool)
...
class TemplateManager: ObservableObject {
    ...
    func loadTemplates() {
        do {
            ...
            AppLogger.error("Failed to load templates: \(error)")
            templates = []
    ...
    func saveTemplates() {
        do {
            ...
            AppLogger.error("Failed to save templates: \(error)")
...
class SubscriptionManager: ObservableObject {
            ...
            case .success(let verification):
                ...
                case .unverified(_, let error):
                    ...
                    await MainActor.run {
                        ...
                        self.isPurchasing = false
                        AppLogger.warning("Transaction verification failed: \(error)")
                    }
                }
    ...
    func updateSubscriptionStatus() async {
        ...
        for await result in Transaction.currentEntitlements {
            ...
            case .unverified(_, let error):
                AppLogger.warning("Unverified transaction: \(error)")
            }
        }
    ...
    private func listenForTransactions() -> Task<Void, Never> {
        ...
        return Task.detached { [weak self] in
            ...
            for await result in Transaction.updates {
                ...
                case .unverified(_, let error):
                    AppLogger.warning("Unverified transaction update: \(error)")
                }
            }
        }
    }
...
struct DeepSeekChatRequest: Codable { let model: String; let messages: [DeepSeekMessage]; let stream: Bool }
        ...
        return AsyncStream { continuation in
            Task {
                do {
                    ...
                    if let systemPrompt { messages.append(DeepSeekMessage(role: "system", content: systemPrompt)) }
                    messages.append(DeepSeekMessage(role: "user", content: userPrompt))
                    let body = DeepSeekChatRequest(model: model, messages: messages, stream: true)
...
class APIService: ObservableObject {
    ...
    init() {
        ...
        AppLogger.debug("[APIService] Bundle key present? \(!bundleKey.isEmpty). Override present? \(!overrideKey.isEmpty). Effective present? \(!effective.isEmpty). Bundle: \(Bundle.main.bundleIdentifier ?? "(nil)")")
        #endif
    }
    ...
    private var effectiveAPIKey: String {
        // Check for runtime override first (for diagnostics/testing)
        ...
        // Use KeychainService which includes fallback to Info.plist in DEBUG
        if let keychainKey = KeychainService.shared.retrieveAPIKey() {
            return keychainKey
        }
    ...
    private func performChat(model: String, messages: [ChatMessageDTO]) async throws -> String {
        ...
        do {
            ...
            if UserDefaults.standard.bool(forKey: "DebugVerboseAPILogging") {
                ...
                AppLogger.debug("DeepSeek request: model=\(model), stream=false, messages=\(messages.count), userMsgs=\(userCount)")
            }
            #endif
            let (data, response) = try await session.data(for: request)
            ...
            if UserDefaults.standard.bool(forKey: "DebugVerboseAPILogging") {
                ...
                AppLogger.debug("DeepSeek response (first 120 chars): \(preview)")
            }
            #endif
            return content
        } catch {
    ...
    func streamChat(model: String, messages: [ChatMessageDTO]) async throws -> AsyncStream<StreamPiece> {
        ...
        do {
            ...
            if UserDefaults.standard.bool(forKey: "DebugVerboseAPILogging") {
                let userCount = messages.filter { $0.role == "user" }.count
                AppLogger.debug("DeepSeek streaming request: model=\(model), stream=true, messages=\(messages.count), userMsgs=\(userCount)")
            }
            #endif
            let (bytes, response) = try await session.bytes(for: request)
            if let http = response as? HTTPURLResponse {
    ...
    func testDeepSeekConnection() async throws -> String {
        let messages = [
            ChatMessageDTO(role: "user", content: "Say hello from NeurospLIT diagnostics test.")
        ]
        return try await performChat(model: "deepseek-chat", messages: messages)
    }
    ...
    func calculateSplit(
        ...
        let result = computeSplitsWrapper(template: template, pool: tipPool)
...
struct NeurospLITApp: App {
    ...
    init() {
        ...
        if !key.isEmpty {
            ...
            AppLogger.info("DeepSeek key found: \(prefix)…")
            ...
            AppLogger.warning("DeepSeek key missing from Info.plist")
            // Initialize alert state for Debug builds
...
struct DiagnosticsView: View {
    ...
    @State private var deepseekOutput: String = ""
    @State private var isLoadingDeepSeek: Bool = false
    @State private var showDeepSeekTransient: Bool = false
    @State private var deepseekKeyPrefix: String = ""
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
                        ...
                        Button("Test Claude Echo") {
                            Task {
                                ...
                                do {
                                    ...
                                    claudeOutput = reply
                            AppLogger.debug("[Diagnostics] Claude reply: \(reply)")
            ...
            GroupBox(label: Label("DeepSeek Test", systemImage: "brain.head.profile")) {
                VStack(alignment: .leading, spacing: 8) {
                    HStack(spacing: 12) {
                        Button("Test DeepSeek Chat") {
                            Task {
                                isLoadingDeepSeek = true
                                defer { isLoadingDeepSeek = false }
                                do {
                                    let reply = try await api.testDeepSeekConnection()
                                    deepseekOutput = "✅ Connected: \(reply)"
                                    AppLogger.debug("[Diagnostics] DeepSeek reply: \(reply)")
                                    AppLogger.debug("[Diagnostics] Key prefix: \(deepseekKeyPrefix)")
                                    withAnimation(.easeInOut(duration: 0.25)) { showDeepSeekTransient = true }
                                    DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
                                        withAnimation(.easeInOut(duration: 0.25)) { showDeepSeekTransient = false }
                                    }
                                } catch {
                                    deepseekOutput = "❌ Failed: \(error.localizedDescription)"
                                    AppLogger.error("[Diagnostics] DeepSeek error: \(error)")
                                    AppLogger.debug("[Diagnostics] Key prefix: \(deepseekKeyPrefix)")
                                    withAnimation(.easeInOut(duration: 0.25)) { showDeepSeekTransient = true }
                                    DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
                                        withAnimation(.easeInOut(duration: 0.25)) { showDeepSeekTransient = false }
                        ...
                        if isLoadingDeepSeek {
                            ProgressView().progressViewStyle(CircularProgressViewStyle())
                        }
                    }
                    if showDeepSeekTransient {
                        Text(deepseekOutput.isEmpty ? "(no output)" : deepseekOutput)
                            .font(.callout)
                            .foregroundColor(deepseekOutput.hasPrefix("✅") ? .green : .red)
                            .lineLimit(4)
                            .transition(.opacity)
                    }
                }
        ...
        .onAppear {
            ...
            let dk = (Bundle.main.infoDictionary?["DEEPSEEK_API_KEY"] as? String)?.trimmingCharacters(in:.whitespacesAndNewlines) ?? ""
            deepseekKeyPrefix = String(dk.prefix(8))
        }
    }
}
...
struct RootView: View {
    ...
    .sheet(isPresented: $showWhipCoins) { WhipCoinsView(showWhipCoins: $showWhipCoins) } // CLEANED
        ...
        .alert(
            ...
            Text("NeurospLIT requires an internet connection for setup and calculations. Please check your connection and try again.")
        }
        #if DEBUG
...
struct WhipCoinsView: View { // CLEANED
    ...
    private let productIDs = [ // CLEANED
        "com.neurosplit.starter", // CLEANED
        "com.neurosplit.standard", // CLEANED
        "com.neurosplit.pro" // CLEANED
    ] // CLEANED
    ...
    private func getCreditsAmount(for product: Product) -> Int { // CLEANED
        ...
        case "com.neurosplit.starter": return 500 // CLEANED
        case "com.neurosplit.standard": return 1100 // CLEANED
        case "com.neurosplit.pro": return 2400 // CLEANED
