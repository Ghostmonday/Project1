...
# Setup Configs/Secrets.xcconfig (primary location used by Xcode)
$secretsPath = "Configs\Secrets.xcconfig"
$secretsExamplePath = "Configs\Secrets.example.xcconfig"

if (!(Test-Path $secretsPath)) {
    if (Test-Path $secretsExamplePath) {
        Copy-Item $secretsExamplePath $secretsPath
        Write-Host "📝 Created Configs\Secrets.xcconfig from template" -ForegroundColor Yellow
        Write-Host "   Please edit Configs\Secrets.xcconfig and add your API keys" -ForegroundColor Yellow
    } else {
        Write-Host "⚠️  Warning: No Configs\Secrets.example.xcconfig found" -ForegroundColor Yellow
    }
} else {
    Write-Host "✅ Configs\Secrets.xcconfig already exists" -ForegroundColor Green
}

# Also setup Configuration/Secrets.xcconfig if needed
$configSecretsPath = "Configuration\Secrets.xcconfig"
$configSecretsExamplePath = "Configuration\Secrets.example.xcconfig"

if (!(Test-Path $configSecretsPath)) {
    if (Test-Path $configSecretsExamplePath) {
        Copy-Item $configSecretsExamplePath $configSecretsPath
        Write-Host "✅ Created Configuration\Secrets.xcconfig" -ForegroundColor Green
    }
}
